...
// Start xpra on server startup to create a display
exec('xpra start :99', (error, stdout, stderr) => {
  if (error) {
  ...
  console.log('Xpra started successfully');
});
...
app.get('/xpra', (req, res) => {
  ...
  const attachCommand = `DISPLAY=:99 XPRA_PASSWORD=${password} xpra attach --speaker=disabled ws://${host}`;
